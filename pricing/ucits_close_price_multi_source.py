...
def usable_close(value: object) -> float | None:
    try:
...
def stooq_symbol(line: dict[str, Any]) -> str:
...
def try_stooq_close(line: dict[str, Any], report_date: date) -> tuple[str, float | None, str | None, str | None, list[str], str]:
    symbol = stooq_symbol(line)
    if not symbol:
        return "fetch_failed", None, None, None, ["missing_provider_symbol_stooq"], symbol
    start = report_date - timedelta(days=35)
    params = {"s": symbol, "i": "d", "d1": start.strftime("%Y%m%d"), "d2": report_date.strftime("%Y%m%d")}
    try:
    ...
    if response.status_code != 200:
        return "fetch_failed", None, None, utc_now(), [f"stooq_http_status:{response.status_code}"], symbol
    text = response.text.strip()
    if not text or text.casefold().startswith("no data"):
        return "fetch_failed", None, None, utc_now(), ["stooq_no_data"], symbol
    try:
        rows = list(csv.DictReader(io.StringIO(text)))
    except Exception as exc:
        return "fetch_failed", None, None, utc_now(), [f"stooq_csv_parse:{type(exc).__name__}"], symbol
    ...
    for row in rows:
        try:
            ...
            continue
        close = usable_close(row.get("Close"))
        if close is not None and row_date <= report_date:
            accepted.append((row_date, close))
    if not accepted:
        return "fetch_failed", None, None, utc_now(), ["stooq_no_usable_completed_close_on_or_before_report_date"], symbol
    row_date, close = max(accepted, key=lambda item: item[0])
    return "priced_non_authoritative", close, row_date.isoformat(), utc_now(), [], symbol


def configure_yfinance() -> object | None:
    try:
...
def try_yfinance_close(
    ...
    *,
    ...
    for attempt in range(1, attempts + 1):
        ...
        if not accepted:
            return "fetch_failed", None, None, utc_now(), ["no_usable_completed_close_on_or_before_report_date"], attempt, rate_limited, stop_batch
        close_date, close_value = max(accepted, key=lambda item: item[0])
        return "priced_non_authoritative", close_value, close_date.isoformat(), utc_now(), blockers, attempt, rate_limited, stop_batch
...
def build_row(
    ...
    *,
    ...
    provider_symbol_stooq = stooq_symbol(line)

    if verification_status == "policy_review_required_not_ucits" or line.get("instrument_type") != "UCITS ETF":
        status, close_price, close_date, observed_at = "policy_review_required_not_ucits", None, None, utc_now()
        blockers = ["not_ucits_policy_review_required"]
    else:
        status, close_price, close_date, observed_at, stooq_blockers, provider_symbol_stooq = try_stooq_close(line, report_date)
        ...
        if status == "priced_non_authoritative":
            ...
            status, close_price, close_date, observed_at, yahoo_blockers, attempt_count, rate_limited, stop_batch = try_yfinance_close(
                yahoo_symbol,
                report_date=report_date,
                pause_seconds=pause_seconds,
                rate_limit_cooldown_seconds=rate_limit_cooldown_seconds,
                max_attempts=max_attempts,
                rate_limit_mode=yahoo_rate_limit_mode,
            )
...
def build_results(
    *,
    ...
    resolved_report_date = parse_report_date(report_date)
    basket = load_yaml(basket_path)
    ...
    if batch_stopped_for_rate_limit:
        ...
        for index, line in enumerate(lines, start=1):
            if line.get("basket_id") in attempted_ids:
                continue
            rows.append({
                "basket_id": line.get("basket_id"), "fund_name": line.get("fund_name"), "isin": line.get("isin"),
                "instrument_type": line.get("instrument_type"), "exchange": line.get("exchange"), "venue_code": line.get("venue_code"),
                "ticker": line.get("ticker"), "provider_symbol_stooq": stooq_symbol(dict(line)),
                "provider_symbol_yahoo": str(line.get("provider_symbol_yahoo") or "").strip(), "currency": line.get("currency"),
                "verification_status": str(line.get("verification_status") or "candidate_requires_verification"), "pricing_status": "fetch_failed",
                "close_date": None, "close_price": None, "source_id": "none", "source_name": "Not attempted after secondary-source throttle",
                "source_quality_status": "non_authoritative_connectivity_only", "source_agreement_status": "not_agreement_gate_not_valuation_grade",
                "observed_at_utc": utc_now(), "requested_report_date": resolved_report_date.isoformat(),
                "completed_close_on_or_before_report_date": False, "valuation_grade": False, "fundable": False,
                "blockers": ["not_attempted_due_to_prior_yahoo_rate_limit"], "request_index": index, "attempt_count": 0,
                "rate_limited": False, "pause_seconds_before_request": 0.0, "rate_limit_cooldown_seconds": rate_limit_cooldown_seconds,
            })
    ...
    payload = {
        ...
        "line_count": len(rows), "priced_line_count": len(priced), "failed_line_count": len(failed),
        "venue_count": len(venues), "currency_count": len(currencies),
        "min_threshold_met": len(rows) >= 8 and len(venues) >= 3 and len(currencies) >= 2,
        "source_chain": ["stooq_daily_csv", "yahoo_yfinance"],
        "throttle_policy": {
            "primary_source": "stooq_daily_csv", "secondary_source": "yahoo_yfinance", "requests_are_serialized": True,
            "pause_seconds_between_symbols": pause_seconds, "yahoo_rate_limit_mode": yahoo_rate_limit_mode,
            "rate_limit_cooldown_seconds": rate_limit_cooldown_seconds, "max_attempts_per_yahoo_symbol": max_attempts,
        ...
        "valuation_grade": False, "funding_authority": False, "portfolio_mutation": False,
        "production_delivery_authority": False, "rows": rows,
    }
    ...
    print(
        ...
        f" | path={out} | report_date={resolved_report_date} | priced={len(priced)} | lines={len(rows)}"
        f" | sources={','.join(payload['source_chain'])} | yahoo_rate_limit={payload['rate_limit_observed']}"
    )
    return out
