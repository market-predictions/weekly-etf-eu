...
def _dedupe_funded_action_rows(section: Tag, positions: dict[str, dict[str, Any]], language: str) -> None:
    ...
    for ticker, position in positions.items():
        ...
        matches = [
            row for row in rows if authoritative_weight in row.get_text(" ", strip=True).casefold()
        ]
        if len(matches) != 1:
            raise RuntimeError(
...
def _enforce_authoritative_client_surface(soup: BeautifulSoup, state: dict[str, Any], language: str) -> None:
    """Supersede stale legacy fragments outside the original activation target sections.

    This is deliberately presentation-only. It derives position count and funded-row
    identity from the protected convergence state and removes retired shadow-policy
    percentages rather than inventing replacement allocation constraints.
    """
    ...
    visible = " ".join(
        section.get_text(" ", strip=True) for section in (section_6, section_13, section_14)
    ).casefold()
    forbidden = (
...
def _install_scoped_stale_copy_validation() -> None:
    ...
    def scoped(soup: BeautifulSoup, state: dict[str, Any], language: str) -> None:
        ...
        # The legacy synchronizer historically raised on stale phrases anywhere in
        # the document after already fixing Sections 1/2/2A. V2 used to narrow that
        # check and thereby allowed stale Sections 6/13/14 through. Supersede those
        # fragments from authoritative state before validating the complete current
        # client surface.
