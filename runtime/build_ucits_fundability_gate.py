...
def _pricing_quality_gate(fund: dict[str, Any], valuation_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return _gate(blockers, {"valuation_row_count": len(valuation_rows), "valuation_grade_row_count": len(valuation_grade_rows), "valuation_statuses": sorted({_as_str(row.get("valuation_status")) for row in valuation_rows})})
...
def write_fundability_gate(registry_path: Path, output_dir: Path, run_id: str, valuation_artifact_path: Path | None = None) -> Path:
    ...
    print("UCITS_FUNDABILITY_GATE_OK" f" | artifact={path}" f" | candidates={artifact['candidate_count']}" f" | not_fundable={artifact['not_fundable_count']}" " | candidate_promotion=false" " | portfolio_mutation=false" " | delivery=false")
    return path
