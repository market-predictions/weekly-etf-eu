...
def _load_registry(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(f"Expected registry mapping in {path}")
    return payload
...
def _registry_line(registry: dict[str, Any], position: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for fund in registry.get("funds") or []:
        if not isinstance(fund, dict) or str(fund.get("isin") or "").strip().upper() != isin:
            continue
        for line in fund.get("trading_lines") or []:
            if not isinstance(line, dict):
                continue
            if str(line.get("exchange_ticker") or "").strip().upper() != ticker:
                continue
            if str(line.get("trading_currency") or "").strip().upper() != currency:
                continue
            if exchange and str(line.get("exchange") or "").strip().lower() != exchange:
                continue
            matches.append((fund, line))
    if len(matches) != 1:
        raise RuntimeError(f"Exact identity binding failed for funded position {isin}/{ticker}/{currency}; matches={len(matches)}")
    return matches[0]
...
def build_normalized_state(
    *,
    ...
    protected = _load_json(portfolio_state_path)
    pricing = load_pricing_artifact(pricing_artifact_path)
    registry = _load_registry(registry_path)
    if str(pricing.get("report_date") or "") != report_date:
        raise RuntimeError(f"Pricing artifact report date mismatch: {pricing.get('report_date')} != {report_date}")

    cash = float(protected.get("cash_eur") or 0)
    if cash < 0:
        raise RuntimeError("Protected cash cannot be negative")
    normalized_positions: list[dict[str, Any]] = []
    invested = 0.0
    for source_row in protected.get("positions") or []:
        if not isinstance(source_row, dict):
            continue
        if source_row.get("investability_status") != "funded_model_position":
            continue
        row = copy.deepcopy(source_row)
        fund, line = _registry_line(registry, row)
        price_row = find_exact_price_row(
            pricing,
            isin=str(row.get("isin") or ""),
            ticker=str(row.get("exchange_ticker") or row.get("ticker") or ""),
            venue_code=str(line.get("venue_code") or ""),
            currency=str(row.get("trading_currency") or "EUR"),
            report_date=report_date,
        )
        shares = int(row.get("shares") or 0)
        if shares <= 0:
            raise RuntimeError(f"Funded position must use positive whole shares: {row.get('ticker')}")
        ...
        historical_allocation = {
            key: row.pop(key)
            for key in _HISTORICAL_TARGET_FIELDS
            if key in row and row.get(key) is not None
        }
        if historical_allocation:
            row["historical_allocation_metadata"] = historical_allocation
        row.update(
            {
                "registry_id": fund.get("registry_id"),
                "identity_binding": "isin_plus_exact_trading_line",
                "identity_binding_valid": True,
                "venue_code": line.get("venue_code"),
                "current_price_local": close,
                ...
                "verification_status": verification_status(price_row),
                "pricing_source": price_row.get("source_name") or price_row.get("source_id"),
                "pricing_source_quality": price_row.get("source_quality_status"),
                "pricing_agreeing_providers": list(price_row.get("agreeing_providers") or []),
                "portfolio_contribution_eur": pnl,
                "unrealized_pnl_eur": pnl,
                "unrealized_pnl_pct": round(pnl_pct, 6) if pnl_pct is not None else None,
                "valuation_source": "thin_current_kernel_exact_line_pricing",
    ...
    if nav <= 0:
        raise RuntimeError("Normalized portfolio NAV must be positive")
    ...
    return {
        ...
        "artifact_type": "weekly_etf_eu_current_normalized_state",
        "run_id": run_id,
        "report_date": report_date,
        "state_valid": True,
        "blockers": [],
        ...
        "sources": {
            "protected_portfolio_state": str(portfolio_state_path),
            "pricing_artifact": str(pricing_artifact_path),
            "identity_registry": str(registry_path),
        },
        "portfolio": {
            "base_currency": protected.get("base_currency") or "EUR",
            "inception_date": protected.get("inception_date"),
            "cash_eur": cash,
        ...
        "pricing_contract": {
            "pricing_authority_mode": "primary_exact_close_plus_optional_independent_verification",
            "exact_date_required": True,
            "identity_binding_required": True,
            "verifier_missing_does_not_invalidate_valid_primary": True,
            "same_date_disagreement_fails_closed": True,
            "source_artifact_gate_passed": True,
        },
    }
