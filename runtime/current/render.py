...
def render_html(state: dict[str, Any], language: str) -> str:
    ...
    positions = "".join(f'<article class="position"><header><strong>{html.escape(_text(r.get("ticker")))}</strong><span>{html.escape(_text(r.get("action")))}</span></header><div class="mini">{_pct(r.get("weight_pct"))} · {_money(r.get("value_eur"))} · {html.escape(_text(r.get("confidence")))}</div><div class="mini"><b>{html.escape(l["fresh"])}:</b> {html.escape(_text(r.get("fresh_cash_view")))}<br><b>{html.escape(l["contribution"])}:</b> {html.escape(_money(r.get("contribution_eur")))}</div><p>{html.escape(_text(r.get("rationale")))}</p><small><b>{html.escape(l["alternative"])}:</b> {html.escape(_text(r.get("best_alternative")))}<br><b>{html.escape(l["trigger"])}:</b> {html.escape(_text(r.get("invalidation_or_next_trigger")))}</small></article>' for r in state.get("funded_position_decisions") or [])
    unresolved = (state.get("epistemics") or {}).get("unresolved") or []
