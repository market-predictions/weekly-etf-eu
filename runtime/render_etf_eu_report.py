...
def render_nl(state: dict[str, Any], report_date: str, proxy_map: Path) -> str:
    cash = _cash(state)
    nav = _nav(state)
...
def render_en(state: dict[str, Any], report_date: str, proxy_map: Path) -> str:
    cash = _cash(state)
    nav = _nav(state)
...
def write_reports(output_dir: Path, state_path: Path, proxy_map: Path, report_date: str) -> tuple[Path, Path]:
    ...
    en_path.write_text(render_en(state, report_date, proxy_map), encoding="utf-8")
    nl_path.write_text(render_nl(state, report_date, proxy_map), encoding="utf-8")
...
def main() -> None:
    ...
    en_path, nl_path = write_reports(Path(args.output_dir), Path(args.state), Path(args.proxy_map), args.report_date)
    print(f"ETF_EU_REPORT_RENDER_OK | en={en_path} | nl={nl_path} | delivery=false")
