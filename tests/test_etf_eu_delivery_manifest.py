...
def test_blocked_design_manifest_is_valid(tmp_path: Path):
    path = write_blocked_design_manifest(
        ...
        fundability_artifact_path="output/fundability/ucits_fundability_gate_20260604_220814.json",
    ...
    assert payload["artifacts"]["fundability_artifact_path"] == "output/fundability/ucits_fundability_gate_20260604_220814.json"
    assert payload["authority"]["email_delivery"] is False
    assert payload["authority"]["pdf_generation"] is False
    assert payload["authority"]["delivery_receipt"] is False
