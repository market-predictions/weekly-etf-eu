...
def _configure_tmp_roots(tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
    ...
    current_runtime = tmp_path / "runtime" / "current"
    ...
    return workflow_dir, controlled
...
def test_governed_transport_is_only_allowed_direct_main_writer(tmp_path: Path, monkeypatch) -> None:
    _, controlled = _configure_tmp_roots(tmp_path, monkeypatch)
...
def test_non_transport_direct_main_write_fails_closed(tmp_path: Path, monkeypatch) -> None:
    workflow_dir, controlled = _configure_tmp_roots(tmp_path, monkeypatch)
    controlled.write_text(_guarded_transport_text(), encoding="utf-8")
