...
from tools.validate_etf_eu_pure_projections import validate_projection_bytes
...
def _require_exact_delivery_head(approved_commit: str, current_head: str) -> None:
    _require(
        current_head == approved_commit,
        "current checkout HEAD differs from approved report commit; later unreviewed changes are not delivery-authorized",
    )
...
def _require_bound_blob(commit_sha: str, path: str, declared_hash: str, label: str) -> bytes:
    ...
    _require(
        _sha256_bytes(data) == _normalise_hash(declared_hash),
        f"{label} hash is not bound to approved report commit",
    )
    return data
...
def _normalise_projection_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(
        str(review_state.get("path") or "") == CANONICAL_REVIEW_STATE_PATH,
        "review_state path is not canonical",
    )
...
def _require_semantic_manifest_stability(candidate_sha: str, approved_commit: str) -> dict[str, Any]:
    ...
    _require(candidate_review_state == approved_review_state, "frozen review_state changed after independent assurance")
...
def validate_git_binding(authority_path: str) -> dict[str, Any]:
    ...
    _require_exact_delivery_head(approved_commit, head)
    ...
    _require(
        str(assurance.get("reviewed_head_sha", "")).lower() == candidate_sha,
        "independent assurance is not bound to assured candidate",
    )
    _require(
        str(assurance.get("expected_base_branch", "")) == expected_base_branch,
        "independent assurance base branch mismatch",
    )
    _require(
        str(assurance.get("expected_base_sha", "")).lower() == expected_base_sha,
        "independent assurance base sha mismatch",
    )
    _require(
        SHA256_RE.fullmatch(str(assurance.get("evidence_binding_sha256", "")).lower()) is not None,
        "independent assurance evidence binding missing",
    )
    ...
    review_state_bytes = _require_bound_blob(approved_commit, review_state_path, review_state_hash, "review_state")

    approved_artifact_bytes: dict[str, bytes] = {}
    for key in REQUIRED_ARTIFACT_KEYS:
        ...
        _require(
            _normalise_hash(item_hash) == _normalise_hash(source_hash),
            f"authority artifact hash differs from approved thin kernel manifest: {key}",
        )
        approved_artifact_bytes[key] = _require_bound_blob(
            approved_commit, item_path, item_hash, f"artifact {key}"
        )

    validate_projection_bytes(review_state_bytes, approved_artifact_bytes)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fail closed unless ETF EU guarded-delivery authority is bound to exact reviewed Git lineage and pure frozen-state projections"
    )
