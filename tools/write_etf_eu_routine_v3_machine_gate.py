...
def language_payload(validation: dict[str, Any], key: str, run_id: str) -> dict[str, Any]:
    data = validation.get(key) if isinstance(validation.get(key), dict) else {}
    passed = data.get("passed") is True
    forbidden = list(data.get("forbidden_tokens") or [])
    blockers = list(data.get("blockers") or [])
    return {
        "schema_version": "etf_eu_routine_pdf_client_grade_v3_adapter_v1",
        "artifact_type": "etf_eu_routine_pdf_client_grade",
        "generated_at_utc": utc_now(),
        "source_run_id": run_id,
        "language": "nl" if key == "dutch" else "en",
        ...
        "funded_surface_consistency_passed": data.get("funded_surface_consistency_passed") is True,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Adapt ETF EU v3 strict validation to routine PDF gate artifacts")
    ...
    dutch = language_payload(validation, "dutch", args.run_id)
    english = language_payload(validation, "english", args.run_id)
    nl_path = output_dir / f"etf_eu_routine_pdf_client_grade_{args.run_id}_nl.json"
    en_path = output_dir / f"etf_eu_routine_pdf_client_grade_{args.run_id}_en.json"
    combined_path = output_dir / f"etf_eu_routine_pdf_client_grade_{args.run_id}.json"
    write(nl_path, dutch)
    write(en_path, english)

    blockers = list(validation.get("blockers") or []) + dutch["blockers"] + english["blockers"]
    combined = {
        "schema_version": "etf_eu_routine_pdf_client_grade_combined_v3",
        "artifact_type": "etf_eu_routine_pdf_client_grade_combined",
        "generated_at_utc": utc_now(),
        "source_run_id": args.run_id,
        "strict_v3_validation": str(validation_path),
        "dutch_artifact": str(nl_path),
        "english_artifact": str(en_path),
        ...
        "pdf_client_grade_passed": validation.get("client_grade_v3_passed") is True and dutch["machine_validation_passed"] and english["machine_validation_passed"],
    ...
    if combined["pdf_client_grade_passed"] is not True or combined["blockers"]:
        raise SystemExit("ETF_EU_ROUTINE_V3_MACHINE_GATE_FAILED")
    print(f"ETF_EU_ROUTINE_V3_MACHINE_GATE_OK | output={combined_path}")
